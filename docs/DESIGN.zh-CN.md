...
重要：原应用已经允许128个 sequence。因此相对 `max_num_seqs=1` 的提升，不能直接叫作“把原应用加速了多少”。正式实验还包括 sequence limit=128、budget=4096 的参考组，用来区分 continuous batching 的收益和进一步调参的收益；其余实验控制仍相同，不称为 vLLM 所有默认参数。
...
所有候选采用同一 **2 requests/s** 的到达过程。初筛每组64个请求；正式比较每组128个请求、至少三个 seed。最初初筛是3个单独 warm-up；发现并发形状 JIT 后，正式组统一改为3轮、每轮8个并发 warm-up 请求，不计入结果。首次启动的模型下载、加载、编译不计入吞吐量。
